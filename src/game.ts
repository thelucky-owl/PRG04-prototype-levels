...
loadCompleted() {
    ...
    this.level1 = new Level1(this.loader.resources['grassTexture'].texture!,this,this.enemyTextures,this.loader.resources['redTexture'].texture!)
    ...
    update(delta:number){
        ...
        if(this.collision(this.player,this.level1.exit)&& this.level1.enemyArray.length <= 0){
            this.level1.destroyAll()
            this.level1 = new Level1(this.loader.resources['grassTexture'].texture!,this,this.enemyTextures,this.loader.resources['redTexture'].texture!)
            this.player.x = 100
        }
        ...
        //check if there are any enmies left
        if(this.underLayer.children.filter((object) => object instanceof Enemy).length === 0){
